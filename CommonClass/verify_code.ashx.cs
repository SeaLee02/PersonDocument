...
namespace Common
{
    ...
    /// verify_code 的摘要说明
    /// 
    /// 验证码用 context.Session["dt_session_code"]  保存在，取的时候也是Session
    /// <img src="verify_code.ashx" />    前面使用   图片标签的路径给这个一般处理程序就可以了
    /// 你点击图片换验证码的时候，需要传个值到这个页面，但你不需要接受      verify_code.ashx?time=" + Math.random()
    /// </summary>
    /// 
